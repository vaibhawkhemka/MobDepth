...
class NYU_BasicAugmentRGBSequence(Sequence):
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        ...
        for i in range(batch_x.shape[0]):
            ...
            y = np.clip(np.asarray(Image.open( BytesIO(self.data[sample[1]]) )).reshape(480,640,1)/255*self.maxDepth,0.1,self.maxDepth)
            ##y = np.clip(np.asarray(Image.open( BytesIO(self.data[sample[1]]) )).reshape(480,640,3)/255*self.maxDepth,0.1,self.maxDepth)
            #print(x.shape)
            #print(y.shape)
